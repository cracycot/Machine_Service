...
public class BasketController {
    ...
    public ResponseEntity<?> addToBasket(@RequestParam Long productId, HttpSession session) throws ProductNotFindException {
        // Попытаться получить корзину из сессии.
        HashMap<Long, Map<String, Object>> basket =
                (HashMap<Long, Map<String, Object>>) session.getAttribute("basket");
        ...
        Product product = productService.getProduct(productId); // Реализовать этот метод.
        if (product != null) {
            Long productIdKey = product.getId();
            // Проверить, есть ли продукт уже в корзине
            if (basket.containsKey(productIdKey) && ((Integer) (basket.get(productIdKey)).get("quantity") < (Integer) (basket.get(productIdKey)).get("inStock"))) {
                // Обновить существующее количество продукта
                Map<String, Object> existingEntry = basket.get(productIdKey);
                existingEntry.put("quantity", (Integer) existingEntry.get("quantity") + 1); // Увеличить количество на 1
            } else if (!basket.containsKey(productIdKey)) {
                // Продукт отсутствует в корзине, добавить новую запись
                Map<String, Object> productDetails = new HashMap<>();
                productDetails.put("quantity", 1); // Количество
                productDetails.put("name", product.getName());
                productDetails.put("category", product.getCategory());
                productDetails.put("article", product.getArticle());
                productDetails.put("price", product.getPrice());
                productDetails.put("inStock", product.getInStock());
                productDetails.put("isNew", product.isNew()); // Поле состояния

                basket.put(productIdKey, productDetails);
            }
        }
        // Вернуть успешный ответ.
        return ResponseEntity.ok().body(Collections.singletonMap("result", "success"));
    ...
    @PostMapping("/basket/increase")
    public ResponseEntity<?> increaseQuantity(@RequestBody BasketUpdateRequestDTO request, HttpSession session) {
        return updateQuantity(session, request.productId(), 1, request.page());
    }
    ...
    private synchronized ResponseEntity<?> updateQuantity(HttpSession session, Long productId, int change, int page) {
        HashMap<Long, Map<String, Object>> basket = (HashMap<Long, Map<String, Object>>) session.getAttribute("basket");
        if (basket == null) {
            basket = new HashMap<>();
            session.setAttribute("basket", basket);
        }
        if (basket.containsKey(productId)) {
            int currentQuantity = (Integer) basket.get(productId).get("quantity");
            int inStock = (Integer) basket.get(productId).get("inStock");

            int newQuantity = currentQuantity + change;

            if (newQuantity > 0 && newQuantity <= inStock) {
                basket.get(productId).put("quantity", newQuantity);
            } else if (newQuantity <= 0) {
            ...
            List<Map<String, Object>> itemsList = new ArrayList<>();
            for (Map.Entry<Long, Map<String, Object>> entry : basket.entrySet()) {
                Map<String, Object> itemMap = new HashMap<>(entry.getValue());
    ...
    @GetMapping("/CleanBasket")
    public ResponseEntity<?> cleanBasket(HttpSession session) {
        session.setAttribute("basket", new HashMap<Long, Map<String, Object>>());
        return ResponseEntity.ok().build();
    }
